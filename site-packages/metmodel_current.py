...
#cb is class for constraint-based models, allows reading/writing of *.sbml files; 
	...
	#other dependencies: glpsol (from glpk), libSBML from http://sbml.org/software/libsbml/, pyparsing (imported in mapGPR.py module)
...
import eq_current		#custom Python modules
...
class cb:
	...
	def write_sbml (self, filename=None):
		...
		for c in self.COMPARTMENTS:
			...
			#except:
			#	print 'problem writing sbml with compartment', c
		...
		for s in self.SPECIES:
			...
			#except:
			#	print 'problem writing sbml with species', s
		...
		for r in self.REACTIONS:
			...
			#except:
			#	print 'problem writing sbml with reaction', r
	...
	def read_sbml(self, filename):
		...
		for reaction in model.getListOfReactions():
			...
			if 'R_ESC_' in id:
				self.ESCAPES.append(id)
			if 'R_EXCH_' in id:
				self.EXCHANGES.append(id)
			
			
			
	def write_dat (self, datfilename, exprfile, minbiomass, gprfile):
		...
		for ID in self.REACTIONS:
			...
			#add non-source, non-escape, and non-exchange reactions to reactionset
			if not 'R_ESC_' in ID and not 'R_SRC_' in ID and not 'R_EXCH_' in ID:
				reactionset[ID] = 1
		...
		hirxns, lowrxns, rxn2expr = mapGPR_current.getRxnExpr(exprfile, gprfile)
